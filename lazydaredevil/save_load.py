...
def get_buildings_n_pois(filename_buildings="data/buildings.json", filename_pois="data/pois.json"):
    buildings = []
    pois = []
    try:
        with open(filename_buildings, 'r') as ofile:
            buildings = json.load(ofile)
        log('Got {:,} buildings'.format(len(buildings)))
    except Exception:
        log('Cannot load data from file "{}"'.format(filename_buildings))
    try:
        with open(filename_pois, 'r') as ofile:
            pois = json.load(ofile)
        log('Got {:,} pois'.format(len(pois)))
    except Exception:
        log('Cannot load data from file "{}"'.format(filename_pois))
    return buildings, pois


def save_buildings_n_pois(buildings, pois, filename_buildings="data/buildings.json", filename_pois="data/pois.json"):
    try:
        with open(filename_buildings, 'w') as ofile:
            json.dump(buildings, ofile)
        log('Saved buildings')
    except Exception:
        log('Cannot save buildings data')
    try:
        with open(filename_pois, 'w') as ofile:
            json.dump(pois, ofile)
        log('Saved pois')
    except Exception:
        log('Cannot save pois data')
